    ...
    <section ref={containerRef} className="relative min-h-screen flex items-center pt-24 pb-12 bg-cream text-dark overflow-hidden">
      ...
      <div className="w-full max-w-7xl mx-auto px-6 relative z-10 flex flex-col lg:flex-row items-center justify-between gap-12 lg:gap-20">
        ...
        <div className="flex-1 flex flex-col items-start text-left pt-10 md:pt-0 order-2 lg:order-1">
          ...
          <h1 className="hero-name flex flex-col text-[12vw] md:text-[6vw] lg:text-[5.5rem] font-display font-black leading-[0.9] tracking-tighter uppercase text-dark mb-4 drop-shadow-sm">
             Mohit <br />
            <span className="text-accent italic font-light drop-shadow-[0_0_15px_rgba(var(--accent-rgb),0.2)] ml-2 md:ml-6 mt-1">Aggarwal</span>
          </h1>
          
          <h2 className="hero-role text-xl md:text-3xl lg:text-4xl font-mono font-medium tracking-tight text-gray-700 mb-8 border-l-4 border-accent pl-4 py-1">
            Full Stack AI Developer
          </h2>
          
          <div className="hero-desc">
            <p className="text-base md:text-lg text-gray-600 max-w-xl mb-6 leading-relaxed">
              I build intelligent applications that blend cutting-edge Artificial Intelligence with elegant, high-performance web and mobile experiences. Turning complex problems into seamless digital products.
            </p>
            
            {/* Tech Tags / Capabilities */}
            <div className="flex flex-wrap items-center gap-2 md:gap-3 mb-10 w-full max-w-xl">
              {['Web Apps', 'Mobile', 'AI Models', 'Backend', 'UI/UX'].map((tag, i) => (
                <span key={i} className="px-4 py-1.5 rounded-sm border border-dark/10 bg-dark/5 backdrop-blur-md text-[10px] md:text-xs font-mono tracking-widest uppercase text-dark/70">
                  {tag}
                </span>
              ))}
            </div>
          ...
          <div className="hero-actions flex flex-wrap items-center gap-4">
            <a href="#projects" className="group relative px-8 py-4 bg-accent text-dark font-display font-bold uppercase tracking-widest text-sm rounded-full overflow-hidden shadow-lg hover:shadow-xl transition-all">
            ...
            <a href="#contact" className="group px-8 py-4 bg-transparent text-dark font-display font-bold uppercase tracking-widest text-sm rounded-full border border-dark/20 hover:border-dark hover:bg-dark/5 transition-colors">
              Contact Me
            </a>
          </div>
        </div>
